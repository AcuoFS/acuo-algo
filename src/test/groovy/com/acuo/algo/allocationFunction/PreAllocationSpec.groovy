...
class PreAllocationSpec extends Specification implements RenjinEval {
  ...
  void "PreAllocation no solver" (){
    ...
    eval('result <- PreAllocation(configurations,algoVersion,callId_vec,callInfo_df,availAsset_df,resource_df,\n' +
    ...
    eval('result_df <- ResultList2Df(result$callOutput_list,callId_vec)')
  ...
  void "PreAllocation with solver"(){
    ...
    that eval('result$callOutput_list$mc1$Amount'), equalTo(c(2293,2459) as SEXP)
    that eval('result$callOutput_list$mc1$NetAmount'), equalTo(c(2109.56,2459) as SEXP)
