...
class resultFormatConversionSpec extends Specification implements RenjinEval {
    void setup() {
        eval("library(stats)")
    ...
    void "ResultMat2CallList converts result matrix to list format by call"() {
        when:
        eval('result_mat <- matrix(c(1000,2000,0,4000), nrow = 2,byrow = T)')
        eval('callInfo_df <- data.frame(\n' +
                            'id=c(\'mc1\',\'mc2\'),\n' +
        ...
        eval('callOutput_list <- ResultMat2CallList(result_mat,callInfo_df,availAsset_df,resource_df)')
        eval('print(callOutput_list)')
        then:
        // check the result list
        that eval('callOutput_list$mc1$Asset'), equalTo(c('EUR','USD') as SEXP)
      }

    void "ResultMat2MsList converts result matrix to list format by statment"() {
      when:
      eval('result_mat <- matrix(c(1000,2000,0,4000), nrow = 2,byrow = T)')
      eval('callInfo_df <- data.frame(\n' +
                    'id=c(\'mc1\',\'mc2\'),\n' +
      ...
      eval('msOutput_list <- ResultMat2MsList(result_mat,callInfo_df,availAsset_df,resource_df)')
      eval('print(msOutput_list)')
      then:
      // check the result list
      that eval('msOutput_list$ms1$Asset'), equalTo(c('EUR', 'USD', 'USD') as SEXP)
    }

    void "ResultVec2Mat converts result vector to matrix format"() {
      when:
      eval('solution_vec <- c(1000,2000,4000,1,1)')
      eval('callId_vec <- c(\'mc1\',\'mc2\')')
      eval('resource_vec <- c(\'EUR---ca2\',\'USD---ca1\')')
      eval('varName_vec <- c(\'ms1___mc1___EUR---ca2\', \'ms1___mc2___EUR---ca2\', \'ms1___mc2___USD---ca1\', \'ms1___dummy___EUR---ca2\', \'ms1___dummy___USD---ca1\')')

      eval('result_mat <- ResultVec2Mat(solution_vec,callId_vec,resource_vec,varName_vec)')
    ...
    void "ResultList2Vec converts result list to vector format"() {
        ...
        eval('result_vec <- ResultList2Vec(allocated_list,callId_vec,minUnit_vec,varName_vec)')
