...
class AllocationScenario2Spec extends Specification implements RenjinEval {
  static {
  ...
  void "AllocationScenario2 no pre-allocation" (){
    when:
  ...
  void "AllocationScenario2 with pre-allocation" (){
    when:
    eval('configurations <- list(debugMode=FALSE)')
    eval('callInfo_df <- data.frame(\n' +
      'id=c(\'mc1\',\'mc2\'),' +
